...
# Parse server flags once so create_app() can read discovery profile; use dummy
# DB paths (tests override DB dependencies so these are only used by lifespan).
_discovery_profile = Path(__file__).resolve().parent / "routes" / "discovery_profile.json"
try:
  config.FLAGS(
    [
      "integration_test",
      "--products_db_path=/tmp/integration_test_products.db",
      "--transactions_db_path=/tmp/integration_test_transactions.db",
      "--port=9999",
      f"--discovery_profile_path={_discovery_profile}",
    ]
  )
except Exception:
  pass  # Flags already parsed (e.g. re-run or test runner)

# Single shared app for all tests (each test sets its own dependency overrides).
_app = create_app()
...
class IntegrationTest(absltest.TestCase):
  ...
  def setUp(self) -> None:
    ...
    # Apply overrides
    self.app.dependency_overrides[dependencies.get_products_db] = (
      override_get_products_db
    )
    self.app.dependency_overrides[dependencies.get_transactions_db] = (
