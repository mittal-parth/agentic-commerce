...
def _discovery_profile_path() -> Path:
  """Path to discovery profile JSON."""
  if FLAGS.discovery_profile_path:
    return Path(FLAGS.discovery_profile_path)
  current_dir = Path(__file__).resolve().parent
  return current_dir / "routes" / "discovery_profile.json"
...
def get_server_version() -> str:
  ...
  profile_path = _discovery_profile_path()
...
try:
  ...
  flags.DEFINE_string(
    "discovery_profile_path",
    None,
    "Optional path to discovery profile JSON (for merchant-specific UCP profile)",
  )
  flags.DEFINE_string(
    "simulation_secret",
    str(uuid.uuid4()),
